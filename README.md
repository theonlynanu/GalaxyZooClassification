...
Also produces dataset-level diagnostic plots (vote distributions, redshift histograms, class balance, etc.) when invoked with `--save` (which I recommend).
...
Note that cross-entropy loss cannot be run on a data split created using `prepare_splits.py` with the `--soft-labels` flag, and KL divergence loss cannot be run on a data split run _without_ the flag.
...
Generates per-image saliency overlays for one or more trained CNN runs. Can target specific images by `--asset-ids` or `--indices`, or auto-select a class-balanced grid via `--auto-select`.
